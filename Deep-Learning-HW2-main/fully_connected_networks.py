...
class Linear_ReLU(object):
    ...
    def backward(dout, cache):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        fc_cache, y_hat = cache
        z, x, w, b = fc_cache
         
        derivative_y_hat = y_hat.clone()
        derivative_y_hat[z == 0] = 1e-8
        derivative_y_hat[z < 0] = 0
        derivative_y_hat[z > 0] = 1
        
        dx = dout * derivative_y_hat @ torch.t(w)
        dx = dx.view(x.shape)
        flattened_x = x.view(x.shape[0], -1)
        dw = torch.t(flattened_x) @ (dout * derivative_y_hat)
        db = torch.sum((dout * derivative_y_hat), dim=0)
...
def softmax_loss(x, y):
    ...
    max_probs = probs[torch.arange(N), y]
    loss = -torch.sum(torch.log(max_probs)) / N
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.w1 = torch.randn(input_dim, hidden_dim, dtype=torch.double) * weight_scale
        self.b1 = torch.zeros(hidden_dim, dtype=torch.double)

        self.w2 = torch.randn(hidden_dim, num_classes, dtype=torch.double) * weight_scale
        self.b2 = torch.zeros(num_classes, dtype=torch.double)

        self.params.update({'W1': self.w1, 'b1': self.b1,
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        N = X.shape[0]
        flattened_X = X.view(N, -1)
        layer1 = flattened_X @ self.params['W1'] + self.params['b1']

        relu1 = layer1.clone()
        relu1[relu1 < 0] = 0

        layer2 = relu1 @ self.params['W2'] + self.params['b2']
        scores = layer2.clone()
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        max_probs = probs[torch.arange(N), y]
        soft_loss = -torch.sum(torch.log(max_probs)) / N

        regularization = self.reg * (torch.sum(self.params['W1']**2) + torch.sum(self.params['W2']**2))
        ...
        grads.update({'W2': torch.t(layer1) @ dout + 2 * self.reg * self.params['W2'],
                      'b2': torch.sum(dout, dim=0)})

        dlayer1 = dout @ torch.t(self.params['W2'])
        dlayer1[layer1 <= 0] = 0

        grads.update({'W1': torch.t(X) @ dlayer1 + 2 * self.reg * self.params['W1'],
                      'b1': torch.sum(dlayer1, dim=0)})
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.randn(input_dim, hidden_dims[0], dtype=self.dtype) * weight_scale
        self.params['b1'] = torch.zeros(hidden_dims[0], dtype=self.dtype)

        for i in range(1, self.num_layers - 1):
            self.params[f'W{i+1}'] = torch.randn(hidden_dims[i - 1],
                                                 hidden_dims[i],
                                                 dtype=self.dtype) * weight_scale
            self.params[f'b{i+1}'] = torch.zeros(hidden_dims[i], dtype=self.dtype)

        self.params[f'W{self.num_layers}'] = torch.randn(hidden_dims[-1], num_classes, dtype=self.dtype) * weight_scale
        self.params[f'b{self.num_layers}'] = torch.zeros(num_classes, dtype=self.dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        flattened_X = X.view(N, -1)
        layer_input = [flattened_X.clone()]
        for i in range(1, self.num_layers):
            lin_layer = layer_input[i-1] @ self.params[f'W{i}'] + self.params[f'b{i}']
            relu1 = lin_layer.clone()
            relu1[relu1 < 0] = 0
            layer_input.append(relu1)

        scores = layer_input[-1] @ self.params[f'W{self.num_layers}'] + self.params[f'b{self.num_layers}']
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        regularization = 0
        for i in range(1, self.num_layers + 1):
            regularization += self.reg * torch.sum(self.params[f'W{i}'] **2)
        ...
        for i in range(self.num_layers, 0, -1):
            grads.update({f'W{i}': torch.t(layer_input[i - 1]) @ dout + 2 * self.reg * self.params[f'W{i}'],
                          f'b{i}': torch.sum(dout, dim=0)})
            dprevious = dout @ torch.t(self.params[f'W{i}'])
            dprevious [layer_input[i - 1] <= 0] = 0
            dout = dprevious
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model,
                    data_dict,
                    update_rule=sgd,
                    optim_config={'learning_rate': 32e-3,},
                    lr_decay=0.90,
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate'] / torch.sqrt(config['cache'] + config['epsilon'])) * dw
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate'] * mhat)/(torch.sqrt(vhat) + config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
